...
- **`,,` 指令**：`,,T/S/J` 切模式、`,,ZH` 注音查碼、`,,TO` 同音字、`,,PYS/PYT` 拼音查碼、`,,SG` 聯想開關、`,,PIN` 固定同碼字排序、`,,V/VT/VS` 剪貼簿。工具列 ⚙ 面板把全部指令做成可點按鈕（不必記指令）
...
- **常用語組字碼**：常用語設定頁可為每句設自訂組字碼，直接打碼叫出、不必開 ♥ 面板；碼已被字表使用時本句排在既有候選之後（設定時即時提示）。密碼與 asciiCapable 欄位自動暫切英文直通
